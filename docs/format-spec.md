...
--- ..  ---
 │   │   └───── no effect
 │   └───────── keep whatever instrument this channel is already using
 └───────────── no note (whatever is playing keeps playing)
```

- **note**: `C-2` … `B-9` (sharps as `C#4`; no flats), `---` = none, `off` = note off.
...
- **instrument**: an instrument name, or `..` = keep the current one.
- **effect**: `---`, or a letter followed by two hex digits (see [Effects](#effects)).
...
[
    "C-4 lead --- | --- ..  --- | C-3 organ --- | C-5 hat ---",
    "skip 3",       // three silent rows: the notes above simply keep ringing
    "off ..   --- | --- ..  --- | --- ..    --- | C-5 hat ---",
]
```
...
instruments (so all three reset to their instruments' volumes). Row 1 is silent in the file, but
