...
pub fn trigger(volume: u8, period: u16) {
    // A note off silences by clearing DAC enable, and a retrigger that reuses
    // the already-loaded table skips upload_wave (the only other place that
    // sets it) — so it must be re-asserted here or the note is inaudible.
