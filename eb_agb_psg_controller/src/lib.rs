...
//! # Runtime-built data
//!
//! [`Track`] and [`Sfx`] are ordinary structs holding `Cow<'static, [_]>`
//! slices, so they can also be built at runtime (an on-device editor, say)
//! rather than by the macros. [`Player::play_song`] and [`Player::play_sfx`]
//! borrow their data for the player's lifetime, which a struct owning both the
//! player and the data cannot satisfy; the optional `dynamic` feature adds
//! [`Player::play_song_shared`] and [`Player::play_sfx_shared`], which take an
//! `Rc` instead. The feature is additive — with it off the player holds a bare
//! reference exactly as before.
//!
//! Data built by hand bypasses the format parser's validation. The player
//! assumes `frames_per_tick > 0` (zero never finishes a frame), `ticks_per_row
//! >= 1`, non-empty rows, notes in `1..=96`, and wave-channel notes no higher
//! than `NOTE_MAX - WAVE_NOTE_OFFSET`.
//!
//! # Hardware notes
//!
...
struct ChannelState {
    ...
    pub vib_offset: i16,
    /// Position in the arpeggio x/y/base cycle. A counter rather than
    /// `tick % 3`, which is a division libcall on ARMv4T.
    pub arp_step: u8,
...
mod tests {
    ...
    static QUIRKS_SONG: Track = include_pmus!("assets/test_hw_quirks.pmus");
    ...
    fn frame_cost_benchmark(gba: &mut agb::Gba) {
        ...
        let mut idle_player = Player::new();
        idle_player.play_song(&TEST_SONG);
        let start = counter();
        ...
        let mut busy_player = Player::new();
        busy_player.play_song(&EFFECTS_SONG);
        let start = counter();
        for _ in 0..ITERATIONS {
            busy_player.frame();
        }
    ...
    /// Register-level regressions: the wave DAC must come back on for a note
    /// that reuses the loaded table (a note off turns it off and the upload is
    /// skipped), square period writes must preserve the length-enable bit, and
    /// panning must not leak from one song into the next.
    #[test_case]
    fn hardware_quirks_survive_effects(_gba: &mut agb::Gba) {
        use crate::registers::{SOUND2CNT_H, SOUND3CNT_L, SOUNDCNT_L};
        const LENGTH_ENABLE: u16 = 1 << 14;
        const WAVE_DAC: u16 = 1 << 7;

        // QUIRKS_SONG is 1 frame per tick, 2 ticks per row.
        let mut player = Player::new();
        player.play_song(&QUIRKS_SONG);

        player.frame(); // row 0: square (with length) + wave trigger, noise pans left
        assert_ne!(SOUND2CNT_H.get() & LENGTH_ENABLE, 0);
        assert_ne!(SOUND3CNT_L.get() & WAVE_DAC, 0);
        assert_eq!(SOUNDCNT_L.get(), 0xF777); // noise right enable (bit 11) off

        player.frame(); // tick 1: V24 writes the square period
        assert_ne!(
            SOUND2CNT_H.get() & LENGTH_ENABLE,
            0,
            "vibrato cleared the length counter"
        );

        player.frame(); // row 1: wave note off turns the DAC off
        assert_eq!(SOUND3CNT_L.get() & WAVE_DAC, 0);

        player.frame(); // row 1 tick 1
        player.frame(); // row 2: same-table wave note must re-enable the DAC
        assert_ne!(
            SOUND3CNT_L.get() & WAVE_DAC,
            0,
            "wave DAC stayed off after a note off"
        );

        // A new song resets the panning state; hardware must follow.
        player.play_song(&TEST_SONG);
        assert_eq!(SOUNDCNT_L.get(), 0xFF77);
    }
